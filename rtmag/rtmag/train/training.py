...
if torch.cuda.is_available():#PCでGPUが使える環境ならばTrueを返す
...
def z_baffa(b):
    ...
    divisor = (np.exp(-K*np.arange(1, b.shape[-2] + 1))).reshape(1, 1, -1, 1).astype(np.float32)
    return divisor
#---------------------------------------------------------------------------------------
def get_dataloaders(args):#torchが読み込める形のデータに変換
    ...
    #batch_sizeは1度に読み込むサンプル数、shuffleは学習ごとにデータをシャッフルするか否か、num_workersは使用するCPUのコア数このサーバーは32(ソケット2つ)、pin_memoryはGPUの転送メモリを固定化するか、drop_lastはバッチで割り切れないデータを無視するか
    val_dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False, 
                                 num_workers=args.data["num_workers"], pin_memory=True)
    #target_fiieldではシャッフルしない

    return train_dataloder, val_dataloader


#---------------------------------------------------------------------------------------
def shared_step(model, sample_batched, args):#「1バッチ分のデータを処理して損失を計算する」ための共通処理
    # input --------------------------------------------------------------
    # [b, 3, x, y, 1]
    #bはバッチサイズ、3はチャンネル数(磁場の3成分)、x, y, 1はグリッド数を表し、境界条件を考えるので1
    inputs = sample_batched['input'].to(device)
    # [b, 3, x, y, 1] -> [b, 1, y, x, 3]
    #PyTorchの可視化ライブラリにベクトルの順番を合わせる、.to(device)でGPUに転送
    inputs = torch.permute(inputs, (0, 4, 3, 2, 1))

    # predict ------------------------------------------------------------
    # [b, 1, y, x, 3] -> [b, 256, y, x, 3]
    outputs = model(inputs).to(device)#256チャネル分の予測値を返す
    ...
    #比較するためにベクトルを変形する、labelsは正解データを意味する
    # [b]
...
#---------------------------------------------------------------------------------------
def criterion(outputs, labels, dx, dy, dz, args):#損失を計算する関数の定義

    loss = {}
    
    # [b, z, y, x, 3] -> [b, z, ...], y, x, 3を1次元にまとめる
    ...
    # mse loss、平均2乗誤差の導出
    mse = MeanSquaredError(num_outputs=opts.shape[-1]).to(device)#ベクトルに対して平均2乗誤差を導出する関数mseの定義、引数を設定することでベクトルに対応
    loss_mse = 0.0
    for i in range(opts.shape[0]):#各バッチの平均2乗誤差を出して平均を取る
        loss_mse += torch.mean(mse(opts[i], lbls[i]))
    loss_mse /= opts.shape[0]
    loss['mse'] = loss_mse

    # ccc loss、 一致相関係数により総損失を評価
    ...
    # denormalization、物理量のスケーリング
    divisor = (1 / np.arange(1, b.shape[-2] + 1)).reshape(1, 1, -1, 1).astype(np.float32)#z軸の配列に対して1, 1/2, 1/3…をかける(1, 1, (1, 1/2…), 1)となるように
    ...
    jx, jy, jz = curl(bx, by, bz, dx, dy, dz)#電流密度を計算
    b = torch.stack([bx, by, bz], -1)
    j = torch.stack([jx, jy, jz], -1)

    # force-free loss
    jxb = torch.cross(j, b, -1)
    loss_ff = (jxb**2).sum(-1) / ((b**2).sum(-1) + 1e-7)#ローレンツ力を2乗して正規化
    loss_ff = torch.mean(loss_ff)#テンソルの損失平均をだす
    loss['ff'] = loss_ff

    # divergence-free loss、磁場の発散の損失
...
#---------------------------------------------------------------------------------------
def eval_plots(b_pred, b_true, b_pot, func, name):#横軸を真の値と予測値の評価値と高さの相関を図で出す
    ...
    if b_pot is not None:#ポテンシャル場がある場合の処理
        plots_B_pot = []
...
#---------------------------------------------------------------------------------------
def val_plot(model, val_dataloader, epoch, args, writer):#モデルの推論結果を可視化してログに残す
    with torch.no_grad():#勾配を考えないことで最適化
        ...
        # [b, 3, x, y, 1] -> [b, 1, y, x, 3]
        inputs = torch.permute(inputs, (0, 4, 3, 2, 1))#順番変更
        # [b, 1, y, x, 3] -> [b, z, y, x, 3]
        outputs = model(inputs)#z=0の値を元に予測
        ...
        # denormalization、正規化解除、 zに[1, 1/2…1/k]をかける
        divi = (b_norm / np.arange(1, b_pred.shape[-2] + 1)).reshape(1, 1, -1, 1).astype(np.float32)
        b_pred = b_pred * divi
        b_true = b_true * divi

        fig1, fig2 = plot_sample(b_pred, b_true, ret=True, v_mm=b_norm/2)#磁場の可視化図を作成
        writer.add_figure('plot/pred', fig1, epoch)#tensorbordに記憶
        writer.add_figure('plot/true', fig2, epoch)
        plt.close()
        
        #-----------------------------------------------------------
        if 'pot' in batch.keys():#ポテンシャル場がある場合の処理
        ...
        fig = eval_plots(b_pred, b_true, b_pot, eval.l2_error, 'rel_l2_err')#評価指標である相対誤差を可視化する
        writer.add_figure(f'plot/rel_l2_err', fig, epoch)
        plt.close()

        fig = eval_plots(b_pred, b_true, b_pot, eval.eps, 'eps')#誤差を評価
        writer.add_figure(f'plot/eps', fig, epoch)
        plt.close()

        torch.cuda.empty_cache()#キャッシュとGPUメモリ開放
        garbage_collection_cuda()
        gc.collect()


#---------------------------------------------------------------------------------------
def val(model, val_dataloader, epoch, args, writer):#モデルの検証
    with torch.no_grad():
        ...
        for i_batch, sample_batched in enumerate(tqdm(val_dataloader, position=1, desc='Validation', leave=False, ncols=70)):#データローダーからバッチ取得
            
            val_loss_dict = shared_step(model, sample_batched, args)#1バッジ分を計算する際の前処理
            ...
            torch.cuda.empty_cache()#解放
            garbage_collection_cuda()
            gc.collect()
        
        total_val_loss /= len(val_dataloader)#平均化
        ...
        writer.add_scalar('val/loss', total_val_loss, epoch)#Tensorbordに計算過程を記録
...
def train(model, optimizer, train_dataloader, val_dataloader, ck_epoch, CHECKPOINT_PATH, args, writer, scheduler=None):
    ...
    for state in optimizer.state.values():#モデルやオプティマイザ内部のテンソルを GPU に移動、オプティマイザは内部に学習率や勾配の情報を持つ
        for k, v in state.items():
            if torch.is_tensor(v):
                state[k] = v.cuda()

    base_path = args.base_path
    n_epochs = args.training['n_epochs']#学習する総エポック
    
    global_step_tmp = len(train_dataloader) * ck_epoch#すでに学習済みのステップを計算
    validation_loss = np.inf#最良判定のため初期値は無限
    for epoch in range(ck_epoch, n_epochs+1):#エポックでループ、ck_epochは再学習用

        if epoch == 0:#学習前にモデル出力を可視化
            with torch.no_grad():
                model = model.eval()
        ...
        with tqdm(train_dataloader, desc='Training', ncols=140) as tqdm_loader_train:#tqdmは進捗バー表示、withを使うことで自動で開放
            for i_batch, sample_batched in enumerate(tqdm_loader_train):#

                tqdm_loader_train.set_description(f"epoch {epoch}")
                
                loss_dict = shared_step(model, sample_batched, args)#「1バッチ分のデータを処理して損失を計算する」ための共通処理
                ...
                loss = args.training['w_mse']*loss_dict['mse'] \
                     ...
                     + args.training['w_div']*loss_dict['div']#総損失
        
                loss.backward()#勾配計算
                optimizer.step()#パラメータ更新
                optimizer.zero_grad()#初期化

                global_step = global_step_tmp + epoch + i_batch#全体の進捗
                tqdm_loader_train.set_postfix(step=global_step)

                total_train_loss += loss.item()#各損失計算
                ...
                writer.add_scalar('step_train/loss', loss.item(), global_step)#tensor_boradに記憶
                ...
                torch.save({'epoch': epoch, 'global_step': global_step, #学習途中の最新モデルを保存、次回学習を再開するときに torch.load で読み込み可能
        ...
        total_train_loss /= len(train_dataloader)#損失平均
        ...
        torch.save({'epoch': epoch, 'global_step': global_step, #学習途中のモデルとオプティマイザ状態を保存
                    'model_state_dict': model.state_dict(), 
                    'optimizer_state_dict': optimizer.state_dict()}, 
                    CHECKPOINT_PATH)
        
        writer.add_scalar('lr', optimizer.param_groups[0]['lr'], epoch)#現在の学習率を記録

        if epoch % args.training['save_epoch_every'] == 0:#定期的にモデル保存
        ...
        if os.path.exists(os.path.join(base_path, "best_model.pt")):#現時点での裁量yモデルを読み込む
            checkpoint = torch.load(os.path.join(base_path, "best_model.pt"))
            validation_loss = checkpoint['validation_loss']

        if total_val_loss < validation_loss:#検証損失がこれまでで最小なら「最良モデル」として保存
